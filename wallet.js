...
async function connectWalletConnect() {
  try {
    ...
    console.error(err);
    showToast("WalletConnect failed");
  }
}

async function switchToArcChain() {
  try {
...
export function showAccountModal() {
  ...
  modal.innerHTML = `
    <div class="glass border border-white/10 rounded-3xl p-8 max-w-sm w-full mx-4 text-center">
      ...
      <button onclick="copyAddress()" class="w-full py-4 rounded-2xl border border-white/20 hover:bg-white/10 mb-3">
        📋 Copy Address
      </button>
      <button onclick="disconnectWallet()" class="w-full py-4 rounded-2xl bg-rose-500/10 text-rose-400 hover:bg-rose-500/20">
        Disconnect Wallet
      </button>
    </div>
  `;
...
export async function disconnectWallet() {
  ...
  signer = null; userAddress = null; provider = null; wcProvider = null;
...
export function showConnectModal() {
      ...
      <div class="glass border border-white/10 rounded-3xl p-8 max-w-md w-full mx-4">
        ...
        <div class="space-y-3">
          <button onclick="connectWallet('metamask')" class="w-full flex items-center gap-4 p-5 rounded-2xl hover:bg-white/10 transition-all">
            ...
            <div class="text-left"><div class="font-semibold text-lg">MetaMask</div></div>
          </button>
  ...
  const oldModal = document.getElementById("connectModal");
